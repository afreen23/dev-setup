#!/bin/bash

echo "Updating the pull secret..."

oc get -n openshift-config secret/pull-secret -ojson | jq -r '.data.".dockerconfigjson"' | base64 -d | jq > secret.json
jq '.auths."quay.io".auth |="cmhvY3MtZW5nOjhRZVUvbDdqL0lLTk9aMEtidU5OYVlHbjdWUmNnVkxlMURrODB2OUhza05VMk4rSWZtNGYxYmJDQVZZMHJObks="' secret.json > temp-auth.json
jq '.auths."quay.io".email |=""' temp-auth.json > temp-secret.json
oc set data secret/pull-secret -n openshift-config --from-file=.dockerconfigjson=temp-secret.json

echo "Pull secret is updated !!!"

rm temp-secret.json temp-auth.json secret.json

echo "Creating catalog source ..."

cat <<EOF | oc create -f -
apiVersion: operators.coreos.com/v1alpha1
kind: CatalogSource
metadata:
  labels:
    ocs-operator-internal: "true"        
  name: ocs-catalogsource
  namespace: openshift-marketplace
spec:
  displayName: OpenShift Container Storage
  icon:
    base64data: 
    mediatype: image/svg+xml
  image: quay.io/rhceph-dev/ocs-registry:4.7.0-268.ci
  publisher: Red Hat
  sourceType: grpc
EOF
